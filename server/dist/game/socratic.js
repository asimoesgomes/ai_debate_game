...
function createEmptyPrepared(topicId, mode) {
    return {
        topicId,
        mode,
        beliefs: [],
...
function nextQuestion(prepared, language) {
    for (const q of exports.SOCRATIC_PLAN) {
        const key = q.key;
        if (key === "summary") {
            if (!prepared.summary.trim())
                return q;
            continue;
        }
        const arr = prepared[key];
...
function addAnswer(prepared, key, answer) {
...
function isComplete(prepared) {
    return (prepared.beliefs.length >= 1 &&
